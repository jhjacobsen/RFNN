...
/* define the input buffer type, e.g. "DSTTYPE" */
#define SRCTYPE double

/* define the output buffer type, should be at least "DSTTYPE" */
#define DSTTYPE double


/* the function prototypes */
template <typename Dtype>
void anigauss(Dtype *input, Dtype *output, int sizex, int sizey,
...
template <typename Dtype>
static void f_iir_xline_filter(Dtype *src, Dtype *dest, int sx, int sy,
    double *filter);

template <typename Dtype>
static void f_iir_yline_filter(Dtype *src, Dtype *dest, int sx, int sy,
    double *filter);

template <typename Dtype>
static void f_iir_tline_filter(Dtype *src, Dtype *dest, int sx, int sy,
    double *filter, double tanp);

template <typename Dtype>
static void f_iir_derivative_filter(Dtype *src, Dtype *dest, int sx, int sy,
	...
	double sigmav, double sigmau, double phi, int orderv, int orderu)
{
    ...
    YvVfilterCoef<Dtype>(sigmax, filter);

    /* filter in the x-direction */
    f_iir_xline_filter<Dtype>(input,output,sizex,sizey,filter);

    /* calculate filter coefficients in tanp-direction */
    YvVfilterCoef<Dtype>(sigmay, filter);

    if (tanp != 0.0) {
        /* filter in the tanp-direction */
         f_iir_tline_filter<Dtype>(output,output,sizex,sizey,filter, tanp);
    }
    else {
        /* isotropic filter or anisotropic filter aligned with grid */
         f_iir_yline_filter<Dtype>(output,output,sizex,sizey,filter);
    }

    /* do the derivative filter: [-1,0,1] rotated over phi */
    for(i=0; i<orderv; i++)
        f_iir_derivative_filter<Dtype>(output, output, sizex, sizey, phirad-PI/2., 1);
    for(i=0; i<orderu; i++)
        f_iir_derivative_filter<Dtype>(output, output, sizex, sizey, phirad, 1);
}
...
template <typename Dtype>
static void f_iir_xline_filter(Dtype *src, Dtype *dest, int sx, int sy, double *filter)
{
    int      i, j;
    double   b1, b2, b3;
    double   pix, p1, p2, p3;
...
template <typename Dtype>
static void f_iir_yline_filter(Dtype *src, Dtype *dest, int sx, int sy, double *filter)
{
    ...
    double phi, int order)
{
   ...
   buf = ( Dtype *) malloc(sx*sizeof(*buf));
