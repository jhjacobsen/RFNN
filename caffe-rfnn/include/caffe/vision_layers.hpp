 ...
 *   */
template <typename Dtype>
class BasisConvolutionLayer : public Layer<Dtype> {
 public:
  explicit BasisConvolutionLayer(const LayerParameter& param)
      : Layer<Dtype>(param) {}
  virtual inline const char* type() const { return "BasisConvolution"; }
  ...
  virtual inline int MinTopBlobs() const { return 1; }
  virtual inline bool EqualNumBottomTopBlobs() const { return true; }
  cv::Mat create_filter(unsigned kernel_h, unsigned kernel_w,float sigma_v,float sigma_u,unsigned order_x, unsigned order_y);
  void saveOutput(const vector<Blob<Dtype>*>& top);

 protected:
  // Added create filter function here
   virtual void Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Forward_gpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
