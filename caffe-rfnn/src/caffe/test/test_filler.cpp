...
template <typename Dtype>
class BasisFillerTest : public ::testing::Test {
 protected:
  BasisFillerTest()
      : blob_(new Blob<Dtype>(1, 1, 51, 51)),
        filler_param_() {
    filler_param_.set_num_basis(10);
    filler_param_.set_sigma(1);
    filler_.reset(new BasisFiller<Dtype>(filler_param_));
    filler_->Fill(blob_);
  }
  virtual ~BasisFillerTest() { delete blob_; }
  Blob<Dtype>* const blob_;
  FillerParameter filler_param_;
  shared_ptr<BasisFiller<Dtype> > filler_;
};

TYPED_TEST_CASE(BasisFillerTest, TestDtypes);

TYPED_TEST(BasisFillerTest, TestFill) {
  EXPECT_TRUE(this->blob_);
  const int count = this->blob_->count();
  const TypeParam* data = this->blob_->cpu_data();
  TypeParam num_basis = 10.;
  TypeParam sigma = 1.;
  TypeParam mean = 0.;
  for (int i = 0; i < count; ++i) {
    mean += data[i];
  ...
  //mean /= count;
  EXPECT_GE(sigma, mean);
  //EXPECT_LE(sigma, mean);
  //TypeParam target_var = this->filler_param_.sigma() * this->filler_param_.sigma();
  //EXPECT_GE(sigma, data[i]);
  //EXPECT_LE(sigma, 5.);
}
